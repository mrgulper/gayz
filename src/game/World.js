...
function buildPark(scene, colliders, solidMeshes) {
  ...
  // the lighting-model cost changes under LOW_QUALITY_MATERIALS.
...
function buildRoom(scene, register, spec) {
  const {
    ...
    // LOW_QUALITY_MATERIALS: cheaper Lambert instead of Standard - already
    // flat-colored either way (no texture map here), so this is purely
    // about the lighting-model cost, not appearance.
...
function placePropSimple(scene, register, fileName, x, z, rotY = 0, scale = 1, collide = true, floorY = 0) {
  ...
  // LOW_QUALITY_MATERIALS: one shared, cheap MeshLambertMaterial for this
  // whole prop instead of a cloned-per-mesh textured PBR material per part -
  // this one function places nearly every piece of furniture/clutter in the
...
function buildOuterZones(scene, register, cullables, towerChestSpots, colliders, solidMeshes, skyscraperShortcuts) {
  ...
  // LOW_QUALITY_MATERIALS: same "one shared flat material per instance"
  // trick as placePropSimple, tinted from the model's own first-mesh color.
