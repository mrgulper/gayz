...
export async function syncPlayerState(sessionId, state) {
  ...
  const { states, zombies, pendingHits, worldEvents, remoteDamage, pickups, chests, vaultOpened, windows, interactions, killEvents, xpGems, host, director, chat } = await _apiCall('sync', { sessionId, playerId, ...state })
  return {
    ...
    killEvents: killEvents || [], xpGems: xpGems || {}, host: host || null, director: director || null, chat: chat || [],
  }
}
