...
- `window.location.reload` can't be reliably stubbed from inside `page.evaluate()` (a plain `window.location.reload = fn` reassignment silently doesn't stick in Chromium) — but calling the real `location.reload()` also doesn't appear to synchronously abort an in-flight `evaluate()` call. Don't try to intercept/count the reload call itself when testing a reload-triggering action (Restore Defaults, Reset Progress, Hardcore respawn); verify the state it wrote (localStorage contents) beforehand instead.
